...
ajaxterm.Terminal_ctor=function(id,width,height) {
	...
	var opt_popup=document.createElement('a');
	var opt_close=document.createElement('a');
...
//		debug("Connection lost timeout ts:"+((new Date).getTime()));
	...
	function mozilla_clipboard() {
		...
		try {
			...
			debug('Access denied, <a href="http://kb.mozillazine.org/Granting_JavaScript_access_to_the_clipboard" target="_blank" rel="noopener">more info</a>');
			return undefined;
		}
...
//			debug(s);
	...
	function init() {
		...
		opt_add(opt_popup,'New');
		opt_add(opt_close,'Close');
		...
		if(opt_color.addEventListener) {
			...
			opt_popup.addEventListener('click',do_popup,true);
			opt_close.addEventListener('click',do_close,true);
		} else {
			opt_get.attachEvent("onclick", do_get);
			opt_color.attachEvent("onclick", do_color);
			opt_paste.attachEvent("onclick", do_paste);
			opt_popup.attachEvent("onclick", do_popup);
			opt_close.attachEvent("onclick", do_close);
		}
		document.onkeypress=keypress;
		document.onkeydown=keydown;
		timeout=window.setTimeout(update,100);
		if (document.width <= 25+(width*7)) {
			document.title=prompt("Window title");
		}
	}
	init();
}
